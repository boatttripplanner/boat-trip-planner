...
const App: React.FC = () => {
  ...
  const hasBlog = true; // O ajusta según lógica real si hay blog
  ...
  return (
    <div className={`min-h-screen bg-slate-100 flex flex-col ${cookieConsent === CookieConsentStatus.PENDING ? 'pb-36 sm:pb-24' : ''}`}>
      ...
      />
      {/* Separador de olas */}
      <div style={{width:'100%', overflow:'hidden', lineHeight:0, marginBottom:'-2px'}}>
        <svg viewBox="0 0 1200 60" fill="none" xmlns="http://www.w3.org/2000/svg" style={{display:'block', width:'100%', height:'60px'}} className="animated-wave">
          <path d="M0 30 Q 300 60 600 30 T 1200 30 V60 H0Z" fill="#b3e0ff" />
        </svg>
      </div>
      <main className="flex-grow container mx-auto p-4 md:p-8 section-animate">
      ...
      <Footer
        ...
        onNavigateToMainApp={() => updateURL(AppView.MAIN_APP)}
        onNavigateToBlogIndex={hasBlog ? () => updateURL(AppView.BLOG_INDEX) : undefined}
        showAds={showAds}
        currentView={currentView}
      />
      {/* Separador de olas antes del footer */}
      <div style={{width:'100%', overflow:'hidden', lineHeight:0, marginTop:'-2px'}}>
        <svg viewBox="0 0 1200 60" fill="none" xmlns="http://www.w3.org/2000/svg" style={{display:'block', width:'100%', height:'60px', transform:'rotate(180deg)'}} className="animated-wave">
          <path d="M0 30 Q 300 60 600 30 T 1200 30 V60 H0Z" fill="#b3e0ff" />
        </svg>
      </div>
